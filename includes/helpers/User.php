...
class WSL_User_Helper
{
    ...
    public function checkUserSteamInfo($return_url)
    {
        $log = (object)[
            'valid' => false,
            'status' => 'logged-out',
            'message' => esc_attr__('Sometinhg happened, please try again', 'easy-steam-account-link'),
            'data' => (object)[]
        ];
        ...
        if (!$log->data->APIKey) {
            $log->message = esc_attr__('Steam User API Key is not defined yet, you must to define an API Key in your wp-admin', 'easy-steam-account-link');
            return $log;
        }

        if (!is_user_logged_in()) {
            $log->message = esc_attr__('You must Login to connect with a Steam Account', 'easy-steam-account-link');
            return $log;
        } else {
